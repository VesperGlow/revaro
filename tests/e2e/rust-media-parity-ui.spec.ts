...
test('old/new 音频章节未使用的 id 缺失或为 null 时仍保留章节', async ({ browser }) => {
  const oldUrl = process.env.E2E_REFERENCE_URL || 'http://127.0.0.1:18180'
  const newUrl = process.env.E2E_NEW_URL || 'http://127.0.0.1:18184'
  const oldContext = await browser.newContext({ viewport: { width: 1440, height: 900 } })
  const newContext = await browser.newContext({ viewport: { width: 1440, height: 900 } })
  ...
  async function exercise(page: Page, baseUrl: string) {
    ...
    await page.route('**/api/files/audio-1/audio', route => route.fulfill({
      json: {
        ...
        cover_url: '',
        chapters: [
          { title: '无编号章节', start: 0, end: 40 },
          { id: null, title: '空编号章节', start: 40, end: 80 },
        ],
      },
    }))
    await open(page, '山间来信.m4a')
  ...
  try {
    ...
    expect(oldResult).toEqual({
      heading: '无编号章节',
      listTitles: ['无编号章节', '空编号章节'],
      chapterCount: 2,
      errorCount: 0,
    })
    expect(newResult, 'Rust 音频章节 id 缺省/null 与 reference 不一致').toEqual(oldResult)
  } finally {
    await Promise.all([oldContext.close(), newContext.close()])
  }
})
...
test('old/new 音频进度成功响应缺少 duration 时仍优先恢复服务端位置', async ({ browser }) => {
  const oldUrl = process.env.E2E_REFERENCE_URL || 'http://127.0.0.1:18080'
  const newUrl = process.env.E2E_NEW_URL || 'http://127.0.0.1:18084'
  const oldContext = await browser.newContext({ viewport: { width: 1440, height: 900 } })
  const newContext = await browser.newContext({ viewport: { width: 1440, height: 900 } })
  ...
  async function exercise(page: Page, baseUrl: string) {
    ...
    await page.route('**/api/files/audio-1/media/progress', async route => {
      if (route.request().method() === 'GET') return route.fulfill({ json: { position: 10 } })
    ...
    const result = Math.floor(await audio.evaluate((element: HTMLAudioElement) => element.currentTime))
    return result
  }
  ...
  try {
    ...
    expect(newResult, 'Rust 音频进度缺少 duration 时未按 reference 恢复服务端位置').toBe(oldResult)
  } finally {
    await Promise.all([oldContext.close(), newContext.close()])
  }
})

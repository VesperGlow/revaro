...
if (process.env.E2E_READER_FLOW === '1') {
  ...
  await page.route('**/*', async (route: Route) => {
    ...
    if (path === `/api/files/${ROOT}/children`) {
      return route.fulfill({
        ...
        body: JSON.stringify({
          items: [{ id: BOOK_ID, parent_id: ROOT, name: bookName, kind: 'file', size: 1000, mime_type: 'application/epub+zip', status: 'ready', created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z' }],
          total_bytes: 1000,
          file_count: 1,
        }),
      })
    ...
    if (path === `/api/files/${ROOT}`) {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ file: { id: ROOT, parent_id: null, name: '我的文件', kind: 'directory', size: 0, status: 'ready', created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z' }, breadcrumbs: [] }) })
    }
    if (path === `/api/files/${BOOK_ID}/thumbnail`) return route.fulfill({ status: 404, body: '' })
    if (path === `/api/files/${BOOK_ID}`) {
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ file: { id: BOOK_ID, parent_id: ROOT, name: bookName, kind: 'file', size: 1000, mime_type: 'application/epub+zip', status: 'ready', created_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z' }, breadcrumbs: [] }) })
    }
...
async function clearClientCache(page: Page) {
  ...
  await page.evaluate(async ({ dbName, bookId }) => {
    localStorage.removeItem(`revaro-reader-manifest:${bookId}`)
    if ('caches' in window) await caches.delete('revaro-reader-flow-v1')
    void dbName
  }, { dbName: DB_NAME, bookId: BOOK_ID })
}
...
test('阅读器顶栏平衡返回、居中标题与实时进度，目录仅从底栏进入', async ({ page }) => {
  ...
  await expect(page.locator('#reader-kind')).toHaveCount(0)
...
test('持久 L2：重开同一本书零 chunk 请求，manifest 版本变化才重取', async ({ page }) => {
  // Playwright's Cache Storage can outlive an incognito context in the shared
  // browser process; start this persistence scenario from a known device state.
  await openReader(page, { clearCache: true })
  ...
  await expect(page.locator('#page-label')).not.toBeEmpty()
  await expect
    .poll(() => Object.keys(flowRequests).sort((a, b) => Number(a) - Number(b)), { timeout: 20_000 })
    .toEqual(['0', '1', '2', '3'])

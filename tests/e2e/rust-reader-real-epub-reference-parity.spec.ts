...
async function login(page: Page) {
  ...
  await page.getByLabel('密码').fill(process.env.REVARO_TEST_PASSWORD || 'revaro-e2e-password')
...
test('三本真实 EPUB：windowSync 后连续翻页不回退到上一页', async ({ page }, testInfo) => {
  ...
  test('reader-real-epub parity suite requires E2E_READER_FLOW=1', ({}, testInfo) => {
    testInfo.skip(true, 'Run with rust-reader-flow-parity.config.ts to execute old/new real EPUB parity');
  });
}

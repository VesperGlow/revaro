...
| `GET` / `DELETE` | `/api/audio-merges/{id}` | 查询进度或取消音频合并任务 |
...
应用识别 JPEG、PNG、WebP、GIF、AVIF，以及 MP4、WebM、OGV、MOV、M4V、MP3、WAV、OGG、M4A、AAC、FLAC、Opus、WMA、AIFF、APE 等常见扩展名。除下述主动音频合并外，应用不会为预览自动转码，最终能否播放仍取决于文件内部编码和访问浏览器的解码能力；兼容性优先推荐 H.264/AAC 的 MP4、WebM，以及 MP3、WAV 或 OGG 音频。

### 音频合并

同时选中 2–256 个音频文件后，文件操作栏会出现“合并音频”。弹窗左侧集中设置输出格式、文件名和可选封面，右侧用上下箭头确认播放顺序。可选择默认 FLAC（无损 `.flac`）、ALAC（无损 `.m4a`，适合 Apple 生态）或 192 kbps AAC（有损 `.m4a`，体积更小）。服务端从 S3 分块存储流式准备源文件，按输入中的最高采样率和最大声道布局统一音频参数，不会像旧实现一样固定降为 48 kHz 立体声；随后交给 FFmpeg 编码，再按 FastCDC 写回当前目录。每个源文件名都会持久化为一个章节标题，封面会嵌入最终成品并用于播放器展示，适合把分轨 ASMR 或有声内容整理成单文件。

合并结果使用分节播放器：可查看章节列表和章节刻度，上一节/下一节跳转、前后快进、倍速播放，并自动保存上次播放位置。播放器通过 HTTP Range 只请求起播或跳转所需的字节，无需先下载完整的数小时文件。为了兼容浏览器，FLAC/ALAC 无损成品会额外生成一份 192 kbps AAC `faststart` 播放副本；下载仍得到原始无损成品，AAC 输出则直接复用成品播放，因此选择无损格式会占用一份额外的播放副本空间。
...
任务在后台运行，关闭弹窗或刷新页面不会中断；页面会轮询准备、编码和入库进度，任务可主动取消。服务器一次只执行一个合并，避免 FFmpeg 与临时磁盘并发打满；最多运行 24 小时。准备阶段和输出阶段会在 `APP_DATA_DIR` 使用临时空间，成功、失败或取消后立即清理。容器重启会清理已中断任务留下的 `pending` 占位文件，原始音频始终保持不变。
...
- 除用户主动发起的音频合并外，无视频转码、波形、OCR 或 EXIF 索引；不受浏览器支持的编码无法直接预览。单文件上限 1 TiB，清单最多 262144 块；超大文件的逐块校验开销随块数线性增长。

...
顶部“离线下载”中心接受 `magnet:` 链接和最大 4 MiB 的 `.torrent` 文件。创建时可以从整棵目录树选择保存位置，元数据就绪后再逐项勾选种子内文件；任务分别显示 BT 下载和 FastCDC 导入阶段的字节进度、速度与当前文件，支持暂停、继续及删除。多文件种子会以种子名称创建根目录，内部相对路径保持不变；单文件种子直接进入所选目录。同名项目不会被覆盖，导入会失败并保留错误记录。
...
当前目录中的图片、GIF 和视频会组成一个循环媒体序列，可使用左右按钮或键盘方向键切换；移动端图片支持左右滑动切换。图片与 GIF 使用原始对象地址，GIF 会保留动画。浏览器原生支持的视频优先走 Range 直放；MKV、AVI、FLV、WMV、MPEG-TS 以及浏览器无法解码的 MP4/MOV 会按需启动 FFmpeg HLS，H.264 只重新封装，HEVC/VP9/AV1 等转成 H.264/AAC，完整进度条可跨尚未生成的区间重新起流跳转。音频文件会打开独立播放器。预览弹窗点击媒体周围的空白区域即可关闭，底部下载按钮始终下载原始文件。
...
- 视频兼容播放是单实例、最多一路的临时 FFmpeg HLS，不生成持久播放副本；低配 CPU 实时转码 1080p HEVC 时可能追不上原速。无波形、OCR 或 EXIF 索引。单文件上限 1 TiB，清单最多 262144 块；超大文件的逐块校验开销随块数线性增长。
...
- 视频缩略图、兼容播放、字幕转换和音频合并依赖容器内置的 ffmpeg/ffprobe（可用 `FFMPEG_PATH` 指定）；超过 512 MiB 的视频不生成缩略图，但仍可在线播放。

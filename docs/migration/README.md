...
```
Go 路径模式 61 条    已实现 34 条    剩余 27 条（其中 1 条为核对脚本的正则噪声）
```

（`/api` 前缀省略；统计时不含正则误匹配的 `Origin`，其余 26 条为真实缺口。）

剩余 26 条按模块：任务系统与 SSE（6：`/tasks`、`/tasks/{id}`、cancel/retry/input、
`/events`）、系统状态与 SSE（2）、分享（2 个路径，含公开 `/s/{token}`）、
下载/预览/批量下载 ZIP（4）、媒体（音频/视频/重探测/字幕，4）、
阅读器（book 信息/资产/封面/进度/flow/分片，6）、缩略图（1）、压缩包解压（1）。

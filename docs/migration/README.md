...
**实现覆盖率（自动核对）**：Go 的 `internal/server/server.go` 注册 74 条路由
（含 `/api` 前缀与 3 条非 API 路由），Rust 端已实现 41 条。剩余 33 条集中在：
任务系统与 SSE（6）、系统状态与 SSE（2）、分享（4，含公开 `/s/{token}`）、
下载与预览与批量下载 ZIP（4）、媒体（音频/视频/重探测/字幕/播放进度，6）、
阅读器（book 信息/资产/封面/进度/flow/分片，7）、缩略图（1）、压缩包解压（1）、
`POST /documents` 已在本轮补上。核对脚本见本节末说明。

...
| 文档编辑器 | 9 | `d18556d`（实现）、`d257696`（E2E）、`2f9eb7b`（editor reverse parity）、`0027c57`（extension/conflict parity）、`fbfb583`（same-value dirty parity）、`65fa5c1`（Markdown mode/security parity） | TXT/Markdown 新建、读取、GFM 预览/HTML 清理、保存、dirty discard、尾随空格校验、错误保留保存、回收站 YAML/Markdown 只读分流和刷新反馈时序已 old/new 实测；新增 11 扩展名入口、延迟 loading、Ctrl+S、ETag 冲突、未保存取消、同值 input dirty 语义、编辑/分栏/预览和安全渲染均已对照；完整视觉、编码/大文件、browser-back 和失败矩阵未完 | `rust-editor-reference-parity.spec.ts` 8/8；其中 old/new 双上下文 5/5，Rust 单实例 bundle 场景 3/3 | 局部 PASS |

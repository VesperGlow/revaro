...
| CRUD 与回收站 | 8 | `d18556d`（实现）、`d257696`（E2E）、`f953af8`（移动失败反馈）、`a46b845`（删除/重命名 parity）、`f82e7f8`（CRUD conflict parity）、`8e72809`（purge error context）、`1abeb30`（empty trash parity）、`83f7738`（成功反馈时序）、`0f87f0d`（重命名关闭时序）、`bf7ad53`（CRUD 键盘边界） | 新建、重命名、移动、复制、删除、恢复、永久删除主链路已 old/new 实测；旧版原始 `files.spec.ts` old/new 各 3/3；新建 API 失败时弹窗关闭/toast、空白输入不发请求、有效名称 Enter 提交、遮罩/Escape 取消、409 关闭确认框、移动 PATCH 失败数量与首项错误文案、删除多选继续处理/刷新清选择、重命名原始空白输入/空名提交/Enter/遮罩/Escape 和 409 保留输入/可重试、800ms PATCH 进行中仍可按 reference 关闭弹窗、回收站恢复/永久删除 409 的项目/选择/文件名错误上下文、清空回收站取消/500 后弹窗列表状态已追加；7 个成功分支已确认等待 reference 的目录/回收站刷新再反馈；完整目录/401/loading/失败矩阵仍未完 | parity Playwright trace、`/tmp/revaro-dialog-error-*`、`rust-transfer-dialog-reference-parity.spec.ts`、`rust-crud-reference-parity.spec.ts` old/new 10/10、`rust-mutation-feedback-order-reference-parity.spec.ts`（old/new 7/7）、旧版原始 files trace | 局部 PASS |
...
| 删除多选与重命名输入边界 | 8、15 | `a46b845`、`f82e7f8`、`8e72809`、`bf7ad53` | `rust-crud-reference-parity.spec.ts` old/new 10/10；`cargo xtask check` 通过 | 双选首项失败/后项成功继续 DELETE、刷新列表、清选择并显示精确反馈；rename 尾随空格原样送入 PATCH；新建文件夹空白/Enter/遮罩/Escape、rename 空名/Enter/遮罩/Escape、409 输入保留/可重试、trash restore/purge 409 的项目/选择/文件名文案均已对照；目录/401/loading/完整失败矩阵仍未完 | 局部 PASS |

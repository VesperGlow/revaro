...
| `[ ]` | 视频播放器 | Range/直接 preview、poster thumbnail、播放/暂停、进度拖动与 seek preview/commit、时间显示、音量/静音、速度、全屏、控制条显隐和自动隐藏一致。 | old/new media parity 已覆盖 poster、controls、速度 Escape、桌面/390/320、touch、损坏响应后的重试和起点/末尾快捷 seek 钳制；视频元数据发现完成后显式启动原始 preview 的 request 时序已恢复（`2bbfd80`）；poster 的 etag 版本参数和特殊字符编码另由 `rust-video-poster-reference-parity.spec.ts` old/new 1/1 验证；音量归零后取消静音恢复最近一次可听音量已由 `rust-media-parity-ui.spec.ts` old/new 1/1 验证（`36ca8d0`）；seek/关闭保存时序及卸载 `keepalive` 语义另由该 spec old/new 1/1 验证；Range/全屏/seek commit 完整矩阵仍待验 |

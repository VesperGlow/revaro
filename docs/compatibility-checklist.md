...
| `[ ]` | 浏览器历史 | 文件夹进入 pushState；返回/前进恢复文件夹/分类；先关闭 modal 再回退页面；stale request 不覆盖新路径。 | old/new 已实际覆盖目录进入、后退、前进 URL 现象、账户弹层后退关闭、普通文件/EPUB/媒体弹层后退关闭并恢复当前文件夹、分享确认框叠加时关闭外层 modal 但保留 reference 外置 dialog、分类筛选后的分类切换/后退/前进、慢/快目录响应竞态，以及两次快速后退按旧版 `popChain` 顺序逐级恢复 loading/内容；`rust-history-sequence-reference-parity.spec.ts` 与相关导航集合 old/new 22/22；其它弹层组合仍待验证 |
...
| `[ ]` | 上传进度 | 单文件/多文件进度、速度、剩余时间、并发、pending/uploading/completing/completed/failed/cancelled 状态和文案一致。 | 旧版进度公式、重试退避和 multipart 裸 PUT 语义已与 Rust 对齐；任务中心传输中隐藏、完成后 100% 和连续失败状态已 old/new 对照；新增 `rust-upload-parity.spec.ts` 以真实四文件选择确认 old/new 并发上限均为 3；完整进度/取消状态矩阵仍待验 |
| `[ ]` | 上传队列 | 队列面板的展开/收起、排序、显示更多、取消、重试、失败原因、完成清理和与任务中心的分工一致。 | old/new 上传集合确认 reference 的可见分工：上传进行中和连续失败的本地任务均不出现在任务中心，完成后才进入完成通知；刷新后无本地句柄的任务中心取消 fallback、真实四文件本地队列并发和页面卸载时不主动删除 session 均已对照。独立本地队列无可见面板；失败/重试和本地取消内部状态矩阵仍待验 |
| `[ ]` | 断点续传 | 刷新/关闭后使用 `revaro.uploads.v1` 恢复；分片获取、记录、complete、abort 和过期记录清理一致。 | old/new 上传集合实际验证 camelCase resume 记录、坏记录过滤、已确认分片跳过、缺失分片重传、record/complete 恢复字段、404 过期 session 清理后重新创建；页面卸载中止本地 XHR 但保留 session 的 reference 语义也已验证；abort、全部已确认分片和关闭后再次选择矩阵仍待验 |
...
| `[ ]` | 复制 | 目标选择、目录/文件、同名处理、任务或立即结果、完成刷新和错误一致。 | old/new 媒体更多菜单实际复制到目标目录，验证旧版保留源文件、POST 目标 `parent_id`、关闭嵌套目录选择器、刷新后副本和“已复制 1 项”反馈；同一入口的 409/500 均关闭弹窗、保留源文件、不显示副本并显示精确失败 Toast；真实 API 两次同名复制均得到 ` - 副本`、` - 副本 2`，无效目标/目录/缺失源状态和错误 envelope 一致（`rust-copy-reference-parity.spec.ts` old/new 3/3，`rust-copy-api-reference-parity.spec.ts` 1/1）；普通文件入口按 reference 不存在，完整 loading/任务刷新矩阵仍待验 |
...
| `[ ]` | 未保存关闭 | dirty 检测、关闭/浏览器后退确认、取消返回编辑、确认丢弃、Esc/backdrop 行为一致。 | old/new 已实际验证编辑后点击关闭或 backdrop 均弹出确认、取消后返回编辑，浏览器后退直接关闭 dirty editor；冲突后仍可放弃，同值 input 仍保持 clean；确认丢弃、Esc 和 readonly 组合矩阵仍待验 |
...
| 状态 | 条目 | 旧版规范与验收点 | 当前 Rust 初检 |
|---|---|---|---|
| `[ ]` | 图片查看 | `/preview`、loading/error、画廊上一张/下一张、缩略图、计数、实际大小/适应窗口、放大缩小、双击、滚轮、拖动边界、stage 点击显隐 chrome、键盘 `←/→/+/-/0/1` 一致。 | old/new media parity 36/36 已覆盖首次打开不预加载、切换时预加载、controls、带 `etag` 版本参数的 thumb、thumb 失败绝对 URL fallback、从根节点 Tab 进入菜单、thumb/menu、退出、缩略图栏切换后保持展开并按 `nearest/center` 定位当前项、桌面/390/320 宽度；新增损坏图片隐藏原图、四项工具 disabled、缩略图保留和“下载原图”下载、低于翻页阈值时拖动跟手位移；滚轮鼠标锚点和 `=`/`-`/`0`/`1`/方向键的缩放、翻页、默认事件语义另由同 spec old/new 1/1 验证；媒体操作菜单下载/移动/复制完整结果另由该 spec old/new 1/1 验证；完整双击/边界矩阵仍待验 |
...
| `[ ]` | 音频播放器 | `/audio` 元数据、封面 fallback、章节、上一/下一章、时间跳转、进度、播放/暂停、loading/error/retry、一首/多首行为一致。 | old/new media parity 已覆盖章节标识、controls、桌面/移动宽度、起点/末尾 seek 钳制；章节抽屉打开后的关闭按钮焦点、Escape/遮罩/重复点击/关闭按钮焦点回收，以及打开/跨章时 reference 的 `scrollIntoView({block:"nearest"})` 已由 `rust-media-parity-ui.spec.ts` 定向 old/new 3/3 验证（`4070c09`）；seek/关闭进度保存时序及关闭时 `keepalive` 最终保存另由该 spec 验证；播放状态、错误重试和完整持久化矩阵仍待验 |
| `[ ]` | 音频持久化 | `revaro-audio-volume`、`revaro-audio-muted`、`revaro-audio-position:{id}`，音量滑块、静音、键盘操作和刷新恢复一致。 | old/new 实际对照暂停后 seek 的 500ms 本地防抖、关闭预览的远端 PUT、音量/静音/位置初始恢复；音量入口在静音/音量为 0 时使用旧版 `VolumeX`、有声时使用 `Volume2`，图标几何已 old/new 1/1 对照（`4070c09`）；卸载最终 PUT 已恢复 `credentials: same-origin`、JSON header、`keepalive: true`（提交 `25a2194`）；连续播放写入、进度接口错误/断线和刷新恢复完整矩阵仍待验，时序修复提交 `f1671c9` |
| `[ ]` | 视频播放器 | Range/直接 preview、poster thumbnail、播放/暂停、进度拖动与 seek preview/commit、时间显示、音量/静音、速度、全屏、控制条显隐和自动隐藏一致。 | old/new media parity 已覆盖 poster、controls、速度 Escape、桌面/390/320、touch、损坏响应后的重试和起点/末尾快捷 seek 钳制；播放中鼠标悬停控制条超过 2.8 秒仍可见已由 old/new 1/1 对照（`8a1da62`）；视频元数据发现完成后显式启动原始 preview 的 request 时序已恢复（`2bbfd80`）；poster 的 etag 版本参数和特殊字符编码另由 `rust-video-poster-reference-parity.spec.ts` old/new 1/1 验证；音量归零后取消静音恢复最近一次可听音量已由 `rust-media-parity-ui.spec.ts` old/new 1/1 验证（`36ca8d0`）；seek/关闭保存时序及卸载 `keepalive` 语义另由该 spec old/new 1/1 验证；视频字幕/设置菜单开合重置控制条 2.8 秒自动隐藏计时器已由 `rust-media-menu-reference-parity.spec.ts` old/new 3/3 验证（修复 `4bf070c`）；Range/全屏/seek commit 完整矩阵仍待验 |
| `[ ]` | 视频字幕 | `/video` metadata、VTT subtitle、选择/关闭字幕、字幕不抖动、加载/解析错误和移动端布局一致。 | old/new media parity 已覆盖默认字幕、选择/关闭、移动端布局与 cue 定位；延迟 preview 初始状态及复杂 cue entity/换行/二级行 class 另由 `rust-media-parity-ui.spec.ts` old/new 各 1/1 验证；有效 cue 后派发 track `error` 时两版均保留当前字幕 overlay（`476d747`）；其余 track 加载/解析错误组合和完整字幕矩阵仍待验 |
| `[ ]` | 视频持久化 | `revaro-video-volume`、`revaro-video-rate`、`revaro-video-position:{id}`，刷新/重开恢复准确且无错误跳 seek。 | old/new 实际对照 `revaro-video-volume`/rate、服务端/本地位置初始恢复，以及自定义 seek 的 600ms 本地防抖和关闭预览远端 PUT；音量滑块归零后点击“取消静音”恢复最近一次可听音量，且 reference 仍保留 localStorage 的 `0`（`36ca8d0`）；零音量打开时原生 `<video>.muted=true` 也已 old/new 对照（`92a4b19`）；卸载最终 PUT 已恢复 `credentials: same-origin`、JSON header、`keepalive: true`（提交 `25a2194`）；连续播放、接口错误/断线和刷新恢复完整矩阵仍待验，时序修复提交 `b625b1c` |
...
| `[ ]` | 不支持/损坏媒体 | unsupported 原文件直接显示旧版错误而不是空白；重试、返回、控制条、错误文案/图标一致。 | old/new 对照不可解码 audio/video 原文件及持续失败到显式重试：错误文案、video“重新尝试”、返回关闭、视频恢复播放和无转码请求一致；新增损坏图片错误层、原图下载和工具 disabled 矩阵；有效 cue 后 track `error` 保留已有字幕 overlay 的行为已 old/new 1/1 对照（`476d747`）；失败后再次失败和完整状态矩阵仍待验 |
...
| `[ ]` | Range 下载/播放 | bytes range、206/416、Content-Range、HEAD/缓存/大文件、音视频 seek 及断点行为与旧版一致。 | old/new `rust-download-reference-parity.spec.ts` 已实际核对固定/open-ended/suffix/zero-suffix、多段 multipart、If-Range、`If-None-Match` 304、malformed/reversed/past-end 416、Content-Range、Content-Length、HEAD；新增阈值以上 multipart 大文件的跨分片/尾部/多段/越界 Range、日期 `If-Modified-Since`/`If-Range` 和带 Range 的 HEAD，均 old/new 2/2 通过。服务端 189/189；音视频 seek/断点和缓存失效完整矩阵仍待补 |
...
| `[ ]` | 归档解压 | 支持格式、密码输入任务、冲突/错误、取消、任务中心、完成刷新和安全路径行为一致。 | old/new `rust-archive-real-reference-parity.spec.ts` 现覆盖真实 ZIP 完成/重复冲突、加密 ZIP waiting_input/任务中心/取消、损坏与 `../` 路径错误；新增真实 TAR、tar.gz/tgz、tar.bz2/tbz2/tbz、tar.xz/txz、tar.zst/tzst、7z 格式，以及真实 RAR 字节（普通文件、符号链接、嵌套目录、空目录）和 `.rar` 后缀识别，均 old/new 比较任务终态、错误文案、输出内容/大小/MIME；RAR 链接拒绝的 `data plane: … (400)` 包装也已恢复。输出矩阵覆盖 TXT/Markdown/JSON/HTML/SVG/未知二进制；单文件 `.gz/.bz2/.xz/.zst` 的 reference 失败保持一致；`entry.txt` 的 Go MIME charset 已恢复（`48c34b3`、`983e0c9`、`6c713a1`）。旧版真实 `/api/tasks/{id}/input` 的锁内持久化死锁未复制到 Rust；Rust server 集成测试覆盖正常错误密码→重试→正确密码完成。running/importing 取消、完整冲突/错误/刷新矩阵仍待验；归档项保持未完成 |
| `[P]` | 分享读取 | 分享状态读取、已存在/不存在、过期/权限、链接显示、复制失败和关闭一致。 | old/new action parity 实际打开 ShareDialog 并读取 inactive/active 状态；复制失败错误段落的旧版无 `role` 语义也已由 `rust-share-dialog-reference-parity.spec.ts` 5/5 对照 |
| `[P]` | 分享创建 | 单文件创建链接、复制 URL、成功/失败、按钮 loading/disabled、公开页面行为和文案一致。 | old/new `rust-actions-parity-ui.spec.ts` 实测创建、复制、重生成和公开读取；创建失败错误段落的旧版无 `role` 语义已恢复，`rust-share-dialog-reference-parity.spec.ts` 5/5 通过 |
...
| `[P]` | `POST /api/files/{id}/copy` | 复制 | Rust `copy_file()` 由 transfer dialog 调用；old/new 实际复制、同名后缀、无效目标、目录/缺失源错误均已验证 |
...
| CRUD 与回收站 | 8 | `d18556d`（实现）、`d257696`（E2E）、`f953af8`（移动失败反馈）、`a46b845`（删除/重命名 parity）、`f82e7f8`（CRUD conflict parity）、`8e72809`（purge error context）、`1abeb30`（empty trash parity）、`83f7738`（成功反馈时序）、`0f87f0d`（重命名关闭时序）、`bf7ad53`（CRUD 键盘边界）、`2c0fcd8`（目录删除） | 新建、重命名、移动、复制、删除、恢复、永久删除主链路已 old/new 实测；旧版原始 `files.spec.ts` old/new 各 3/3；新建 API 失败时弹窗关闭/toast、空白输入不发请求、有效名称 Enter 提交、遮罩/Escape 取消、409 关闭确认框、移动 PATCH 失败数量与首项错误文案、删除多选继续处理/刷新清选择、目录删除取消/确认/刷新、重命名原始空白输入/空名提交/Enter/遮罩/Escape 和 409 保留输入/可重试、800ms PATCH 进行中仍可按 reference 关闭弹窗、回收站恢复/永久删除 409 的项目/选择/文件名错误上下文、清空回收站取消/500 后弹窗列表状态已追加；7 个成功分支已确认等待 reference 的目录/回收站刷新再反馈；完整目录/401/loading/失败矩阵仍未完 | parity Playwright trace、`/tmp/revaro-dialog-error-*`、`rust-transfer-dialog-reference-parity.spec.ts`、`rust-crud-reference-parity.spec.ts` old/new 11/11、`rust-mutation-feedback-order-reference-parity.spec.ts`（old/new 7/7）、旧版原始 files trace | 局部 PASS |
...
| 文档编辑器 | 9 | `d18556d`（实现）、`d257696`（E2E）、`2f9eb7b`（editor reverse parity）、`0027c57`（extension/conflict parity）、`fbfb583`（same-value dirty parity）、`65fa5c1`（Markdown mode/security parity）、`dc0b82d`（visual parity）、`9229619`（dirty close/history parity）、`fcd771c`（blank document parity）、`a82441c`（create failure parity）、`1b93087`（Markdown hard-break parity） | TXT/Markdown 新建、读取、GFM 预览/HTML 清理、保存、dirty discard、尾随空格校验、错误保留保存、回收站 YAML/Markdown 只读分流和刷新反馈时序已 old/new 实测；新增 11 扩展名入口、空文档未保存标记/关闭、创建 409 后 editor/错误/重试、延迟 loading、Ctrl+S、ETag 冲突、未保存取消、同值 input dirty 语义、编辑/分栏/预览和安全渲染、hard break 等边界 DOM、关闭按钮/backdrop 取消和 dirty browser-back 均已对照；1440×900/390×844 编辑器几何与 computed style 视觉矩阵已通过；编码/大文件、Esc/确认丢弃、readonly 组合和失败矩阵未完 | `rust-editor-reference-parity.spec.ts` old/new 12/12；`rust-editor-visual-reference-parity.spec.ts` old/new 1/1；Markdown 单测 3/3；其中 old/new 双上下文视觉 1/1，Rust 单实例 bundle 场景 3/3 | 局部 PASS |
| 阅读器 | 10 | `14084bf`（core）、`d18556d`（web）、`ed13571`（全局 block）、`a47dc50`（定位/进度/缓存/导航 E2E）、`94ba14b`（loading/error parity）、`cfaea4d`（完整 reader-flow）、`b1cf940`（真实 EPUB parity）、`a0b5975`（偏好边界/touch-cancel parity） | old/new reference reader-flow 各 17/17；专用双项目配置中的真实 EPUB 各 1/1；`rust-reader-reference-parity.spec.ts` 现 5/5，覆盖默认 19/1.7/light、字号 14/32 边界与重开恢复、TOC/排版/阅读器 Escape 分层、390×844 横向/纵向/touch-cancel；全局 block 0…37、14/14.0% 进度文案、TOC Escape 焦点、L2 同版本零请求/版本变化重取、loading/error 结构与关闭、真实 fixture 连续翻页/windowSync 均已实测；断网/失败/并发保存和完整 UI 状态矩阵仍未完 | `rust-reader-flow-reference-parity.spec.ts`、`rust-reader-real-epub-reference-parity.spec.ts`、`rust-reader-reference-parity.spec.ts`（5/5）、reader-flow/real EPUB trace、`rust-reader-ui.spec.ts`、`rust-open-item-reference-parity.spec.ts` | 局部 PASS |
| 媒体 | 11 | `d18556d`（实现）、`d257696`（E2E）、`b84ce18`（thumb/focus parity）、`dcfc9c1`（video poster etag parity）、`f1671c9`（audio seek persistence timing）、`b625b1c`（video seek persistence timing）、`22f320e`（subtitle text parity）、`7119137`（image wheel anchor）、`25a2194`（卸载 keepalive 进度保存）、`df134c1`（touch gesture parity）、`710e434`（unsupported parity）、`2bbfd80`（损坏/seek 边界与视频启动时序 parity）、`2f70d29`（chapter index parity）、`36ca8d0`（视频音量恢复 parity）、`b85d0ad`（隐藏控制无障碍 parity）、`7f08739`（seek parity 采样同步）、`d9edd80`（图片拖动跟手 parity）、`f22bc2b`（胶片栏状态/定位 parity）、`e79af9f`（音视频挂载焦点 parity）、`4070c09`（音频抽屉/音量 parity）、`92a4b19`（视频零音量 muted parity）、`8a1da62`（视频 hover parity）、`476d747`（字幕 track error parity）、`94ace8d`（媒体操作菜单/尺寸文案 parity）、`faec61d`（图片键盘 parity）、`9201c28`（视频键盘 parity） | 图片/音频/视频桌面/窄屏/触摸、字幕、存储、全屏主链路、缩略图版本参数和预览 Tab 首焦点已有 old/new 实测；音视频播放器挂载焦点的 `preventScroll` 参数也已在 old/new 双版本记录并对齐；旧版原始 `media-ui.spec.ts` old/new 各 10/10；视频 poster 的 etag 编码、音频非连续 chapter id 下按数组位置的索引/编号、音频/视频 seek 与关闭保存时序、复杂字幕 entity/换行、图片滚轮锚点、音视频卸载时 `keepalive: true` 最终进度保存、390×844 触摸手势、不可解码原文件错误分流、损坏图片状态/下载、显式视频重试和起点/末尾 seek 边界、视频音量归零后取消静音恢复最近一次可听音量、零音量打开时 video 原生 `muted`、视频控制条隐藏时的 `inert` 与 range `aria-valuetext`、低于翻页阈值时图片拖动跟手位移、缩略图栏切换后的展开保持和 `nearest/center` 定位、音频章节抽屉焦点/重复开关/当前章节 `block:"nearest"` 定位、音量静音图标几何、有效 cue 后 track `error` 保留当前字幕 overlay、媒体图片/音频/视频操作菜单的下载/移动/复制完整结果及 metadata 尚未提供尺寸时不显示 `0 × 0`、图片 `=`/`-`/`0`/`1`/方向键与视频 Space/`k`/左右方向键/`m` 快捷键及默认事件语义均已单独 old/new 验证；完整字幕失败、Range/失败重试、音频播放/错误/持久化和视觉状态矩阵仍未完 | `rust-media-parity-ui.spec.ts` old/new 37/37；另有 `rust-media-focus-reference-parity.spec.ts` old/new 2/2、旧版原始 `media-ui.spec.ts` old/new 各 10/10、`rust-video-poster-reference-parity.spec.ts` old/new 1/1；最新视频键盘定向 old/new 1/1，图片键盘 1/1、字幕 track error 1/1、隐藏控制 accessibility 定向重复 5/5、媒体操作菜单定向 old/new 1/1；`cargo xtask check`、`cargo xtask web-build`、`cargo fmt --all -- --check` 通过 | 局部 PASS |
| 下载/分享/归档 | 12 | `ff43716`（Range）、`74051ee`（ServeContent Range parity）、`85d4a50`（批量 ZIP ID/完整 E2E）、`724aa3d`（归档真实错误/冲突/取消 parity）、`48c34b3`（归档格式/MIME parity）、`983e0c9`（归档输出 MIME 矩阵）、`6c713a1`（真实 RAR 错误 parity）、`d18556d`（UI）、`d257696`（E2E）、`3967289`（公开分享 transport E2E）、`7b278b7`（归档入口双版本） | 单文件、批量 ZIP、分享生命周期、公开分享安全 headers/Range/无效 token、归档入口/任务密码/状态刷新、preview/206/416 已 old/new 实测；ServeContent 的多段 Range、错误边界已恢复；归档真实 ZIP 的重复输出、等待态取消、损坏/穿越错误语义、多格式任务终态/输出/MIME 和真实 RAR 链接拒绝错误包装已恢复；大文件/缓存条件请求/媒体 seek、running/importing 取消和归档完整状态矩阵仍未完 | download/share/action parity trace；`rust-download-reference-parity.spec.ts` old/new 2/2（含 multipart 大文件/日期条件）；`rust-batch-download-reference-parity.spec.ts` old/new 1/1（真实 ZIP 内容、重复命名、一次性 token、错误分流）；公开分享 old/new 追加断言；`rust-archive-reference-parity.spec.ts` old/new 1/1；`rust-archive-real-reference-parity.spec.ts` real ZIP/密码/错误/安全 4/4、格式矩阵 1/1、真实 RAR 1/1 | 局部 PASS |
...
| 分享弹窗 loading/active/二级确认/复制状态 | 2、12、15 | `01c48cb`、`39b4055`、`94ad574`、`c1ce934`、`31ca8e5`、`4940aa2`、`30ac97c` | `rust-share-dialog-reference-parity.spec.ts` old/new 双上下文 5/5；`cargo xtask check` 通过 | 延迟读取/创建请求期间实际比较可关闭 loading、遮罩、重新打开、active 链接输入、按钮状态、尺寸和文案；二级确认取消/提交关闭/错误回显、复制成功/失败局部状态、active Escape、重生成成功不产生全局 toast、停止分享产生成功 toast 以及失败提示无额外 `role` 均已对照；分享非 trap 边界已补齐，完整视觉状态矩阵仍未完 | 局部 PASS |

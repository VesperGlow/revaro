...
| 状态 | 条目 | 旧版规范与验收点 | 当前 Rust 初检 |
|---|---|---|---|
| `[ ]` | 图片查看 | `/preview`、loading/error、画廊上一张/下一张、缩略图、计数、实际大小/适应窗口、放大缩小、双击、滚轮、拖动边界、stage 点击显隐 chrome、键盘 `←/→/+/-/0/1` 一致。 | old/new media parity 27/27 已覆盖首次打开不预加载、切换时预加载、controls、带 `etag` 版本参数的 thumb、thumb 失败绝对 URL fallback、从根节点 Tab 进入菜单、thumb/menu、退出、桌面/390/320 宽度；新增损坏图片隐藏原图、四项工具 disabled、缩略图保留和“下载原图”下载、低于翻页阈值时拖动跟手位移；滚轮鼠标锚点另由同 spec old/new 1/1 验证；完整键盘/边界矩阵仍待验 |

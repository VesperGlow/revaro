...
| `[ ]` | Range 下载/播放 | bytes range、206/416、Content-Range、HEAD/缓存/大文件、音视频 seek 及断点行为与旧版一致。 | old/new `rust-download-reference-parity.spec.ts` 已实际核对固定/open-ended/suffix/zero-suffix、多段 multipart、If-Range、malformed/reversed/past-end 416、Content-Range、Content-Length、HEAD；服务端 189/189。大文件、缓存条件请求和媒体 seek/断点仍待补 |
...
| 下载 Range 响应与 `ServeContent` 错误边界 | 12、13、15 | `74051ee` | `rust-download-reference-parity.spec.ts` old/new 1/1；`cargo test -p revaro-server` 189/189 | 真实上传同一文本文件，比较固定/open-ended/suffix/zero-suffix、多段 `multipart/byteranges`、匹配/失配 `If-Range`、malformed/reversed/past-end 416、Content-Range/Length、HEAD；Rust 初始 Range parser 的 4 处差异已恢复；大文件/缓存条件请求/媒体 seek 仍未闭环 | 局部 PASS |

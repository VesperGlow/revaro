...
| `[ ]` | 归档解压 | 支持格式、密码输入任务、冲突/错误、取消、任务中心、完成刷新和安全路径行为一致。 | old/new `rust-archive-real-reference-parity.spec.ts` 已实际完成同一未加密 ZIP 的列表入口→确认→任务完成→输出目录/子文件结果对照（2/2），并对照同一加密 ZIP 的 waiting_input 任务、任务中心“等待输入密码”入口和密码弹窗结构（2/2）；`rust-archive-reference-parity.spec.ts` 继续覆盖错误密码/输入后状态刷新。旧版基线的 `/api/tasks/{id}/input` 在真实运行中会卡在 `resumeWithPassword` 锁内调用持久化的历史死锁，未将该缺陷复制到 Rust；Rust server 集成测试覆盖正常错误密码→重试→正确密码完成。冲突/错误/取消、支持格式和完整刷新矩阵仍待验 |
...
| 下载/分享/归档 | 12 | `ff43716`（Range）、`74051ee`（ServeContent Range parity）、`85d4a50`（批量 ZIP ID/完整 E2E）、`d18556d`（UI）、`d257696`（E2E）、`3967289`（公开分享 transport E2E）、`7b278b7`（归档入口双版本） | 单文件、批量 ZIP、分享生命周期、公开分享安全 headers/Range/无效 token、归档入口/任务密码/状态刷新、preview/206/416 已 old/new 实测；ServeContent 的多段 Range、错误边界已恢复；大文件/缓存条件请求/媒体 seek 与视觉状态仍未完 | download/share/action parity trace；`rust-download-reference-parity.spec.ts` old/new 2/2（含 multipart 大文件/日期条件）；`rust-batch-download-reference-parity.spec.ts` old/new 1/1（真实 ZIP 内容、重复命名、一次性 token、错误分流）；公开分享 old/new 追加断言；`rust-archive-reference-parity.spec.ts` old/new 1/1；`rust-archive-real-reference-parity.spec.ts` 真实 ZIP old/new 2/2 | 局部 PASS |

...
| 下载/分享/归档 | 12 | `ff43716`（Range）、`74051ee`（ServeContent Range parity）、`d18556d`（UI）、`d257696`（E2E）、`3967289`（公开分享 transport E2E）、`7b278b7`（归档入口双版本） | 单文件、ZIP、分享生命周期、公开分享安全 headers/Range/无效 token、归档入口/任务密码/状态刷新、preview/206/416 已 old/new 实测；ServeContent 的多段 Range、错误边界已恢复；大文件/缓存条件请求/媒体 seek 与视觉状态仍未完 | download/share/action parity trace；`rust-download-reference-parity.spec.ts` old/new 1/1；公开分享 old/new 追加断言；`rust-archive-reference-parity.spec.ts` old/new 1/1；`rust-archive-real-reference-parity.spec.ts` 真实 ZIP old/new 2/2 | 局部 PASS |
...
| 下载 Range 响应与 `ServeContent` 错误边界 | 12、13、15 | `74051ee`、`70b2045` | `rust-download-reference-parity.spec.ts` old/new 1/1；`cargo test -p revaro-server` 189/189 | 真实上传同一文本文件，比较固定/open-ended/suffix/zero-suffix、多段 `multipart/byteranges`、匹配/失配 `If-Range`、匹配/通配/过期 `If-None-Match` 304、malformed/reversed/past-end 416、Content-Range/Length、HEAD；Rust 初始 Range parser 和条件响应差异已恢复；大文件/日期条件/媒体 seek 仍未闭环 | 局部 PASS |

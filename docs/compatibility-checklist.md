...
| 阅读器 | 10 | `14084bf`（core）、`d18556d`（web）、`ed13571`（全局 block）、`a47dc50`（定位/进度/缓存/导航 E2E）、`94ba14b`（loading/error parity）、`cfaea4d`（完整 reader-flow）、`b1cf940`（真实 EPUB parity） | old/new reference reader-flow 各 17/17；专用双项目配置中的真实 EPUB 各 1/1；全局 block 0…37、14/14.0% 进度文案、TOC Escape 焦点、L2 同版本零请求/版本变化重取、loading/error 结构与关闭、真实 fixture 连续翻页/windowSync 均已实测；触摸/偏好和完整 UI 状态矩阵仍未完 | `rust-reader-flow-reference-parity.spec.ts`、`rust-reader-real-epub-reference-parity.spec.ts`、reader-flow/real EPUB trace、`rust-reader-ui.spec.ts`、`rust-open-item-reference-parity.spec.ts` | 局部 PASS |

...
| 移动/复制 loading、失败与成功反馈 | 8、15 | `8bbbd22`、`f953af8`、`83f7738`、当前工作区复制 parity 用例待提交 | `rust-transfer-dialog-reference-parity.spec.ts` old/new 双上下文 2/2；`rust-mutation-feedback-order-reference-parity.spec.ts` old/new 7/7；新增 `rust-copy-reference-parity.spec.ts` old/new 1/1；`cargo xtask check` 通过 | 延迟移动 PATCH 请求期间实际点击 old/new 遮罩，比较弹窗卸载结果；同一 500 响应比较“已移动 0 项，1 项失败：文件：move failed”；700ms children 延迟确认成功 toast 等待目录刷新；新增按旧版媒体更多菜单入口完成复制、目标目录选择、源文件保留、副本刷新和“已复制 1 项”；Rust 初始 busy 限制、失败数量文案和成功反馈时序均已恢复；目标排除、冲突和完整错误矩阵仍未完 | 局部 PASS |

...
| `[ ]` | 不支持/损坏媒体 | unsupported 原文件直接显示旧版错误而不是空白；重试、返回、控制条、错误文案/图标一致。 | old/new 对照不可解码 audio/video 原文件及持续失败到显式重试：错误文案、video“重新尝试”、返回关闭、视频恢复播放和无转码请求一致；新增损坏图片错误层、原图下载和工具 disabled 矩阵；字幕 track 错误、失败后再次失败和完整状态矩阵仍待验 |

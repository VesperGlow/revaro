...
| `[ ]` | 复制 | 目标选择、目录/文件、同名处理、任务或立即结果、完成刷新和错误一致。 | old/new 媒体更多菜单实际复制到目标目录，验证旧版保留源文件、POST 目标 `parent_id`、关闭嵌套目录选择器、刷新后副本和“已复制 1 项”反馈；普通文件入口按 reference 不存在，同名/失败仍待验 |
...
| 预览 HTTP 与 thumbnail 回退 | 12、13、15 | `待提交` | `rust-preview-reference-parity.spec.ts` old/new 1/1 | 真实上传 TXT/PNG/WAV/WebM，逐项对照 preview 类型、inline disposition、Range/If-Range、415/401 错误、图片 thumbnail 与无 thumbnail 分支；独立实例的 ETag 值及 JPEG 编码长度按语义比较 | PASS |
...
| 移动/复制 loading、失败与成功反馈 | 8、15 | `8bbbd22`、`f953af8`、`83f7738`、`8c12a23`（复制 parity） | `rust-transfer-dialog-reference-parity.spec.ts` old/new 双上下文 2/2；`rust-mutation-feedback-order-reference-parity.spec.ts` old/new 7/7；`rust-copy-reference-parity.spec.ts` old/new 1/1；`cargo xtask check` 通过 | 延迟移动 PATCH 请求期间实际点击 old/new 遮罩，比较弹窗卸载结果；同一 500 响应比较“已移动 0 项，1 项失败：文件：move failed”；700ms children 延迟确认成功 toast 等待目录刷新；按旧版媒体更多菜单入口完成复制、目标目录选择、源文件保留、副本刷新和“已复制 1 项”；Rust 初始 busy 限制、失败数量文案和成功反馈时序均已恢复；目标排除、冲突和完整错误矩阵仍未完 | 局部 PASS |

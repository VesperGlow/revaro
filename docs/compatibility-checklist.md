...
- `[P]` 当前兼容恢复工作树 HEAD：`31a8ce7`；上面的 `e9b6202` 保留为恢复开始时的 Rust 基线，后续每个逻辑模块均以独立提交推进。
...
| 状态 | 条目 | 旧版规范与验收点 | 当前 Rust 初检 |
|---|---|---|---|
| `[ ]` | 文件卡/行 | 文件名、大小、类型、更新时间、目录/媒体/文档标识、thumbnail/cover、fallback 和截断规则一致；方块与列表都验证。回收站目录按 Enter 仍阻止默认事件但不打开。 | Rust 有 FileTile/rows 基础；回收站目录 Enter 的事件语义已恢复，视觉和全类型矩阵待对照 |
...
| 文件项回收站键盘语义 | 6、15 | `31a8ce7` | `rust-file-interaction-parity.spec.ts` old/new 定向各 1/1 | 实际创建并删除目录后聚焦回收站网格卡按 Enter；两版均 `defaultPrevented=true` 且不打开目录，覆盖 FileTile/FileRow 的无条件 `.prevent` 语义 | 局部 PASS |

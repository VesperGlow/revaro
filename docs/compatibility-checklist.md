...
| 文档编辑器 | 9 | `d18556d`（实现）、`d257696`（E2E）、`2f9eb7b`（editor reverse parity）、`0027c57`（extension/conflict parity）、`fbfb583`（same-value dirty parity）、`65fa5c1`（Markdown mode/security parity）、`dc0b82d`（visual parity）、`9229619`（dirty close/history parity）、`fcd771c`（blank document parity）、`a82441c`（create failure parity） | TXT/Markdown 新建、读取、GFM 预览/HTML 清理、保存、dirty discard、尾随空格校验、错误保留保存、回收站 YAML/Markdown 只读分流和刷新反馈时序已 old/new 实测；新增 11 扩展名入口、空文档未保存标记/关闭、创建 409 后 editor/错误/重试、延迟 loading、Ctrl+S、ETag 冲突、未保存取消、同值 input dirty 语义、编辑/分栏/预览和安全渲染、关闭按钮/backdrop 取消和 dirty browser-back 均已对照；1440×900/390×844 编辑器几何与 computed style 视觉矩阵已通过；编码/大文件、Esc/确认丢弃、readonly 组合和失败矩阵未完 | `rust-editor-reference-parity.spec.ts` 11/11；`rust-editor-visual-reference-parity.spec.ts` old/new 1/1；其中 old/new 双上下文视觉 1/1，Rust 单实例 bundle 场景 3/3 | 局部 PASS |

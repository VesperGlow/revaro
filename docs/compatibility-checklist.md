...
- `[P]` 当前兼容恢复工作树 HEAD：`9ff563b`；上面的 `e9b6202` 保留为恢复开始时的 Rust 基线，后续每个逻辑模块均以独立提交推进。
...
| 目录选择器图标、展开控件与 Escape 语义 | 8、15 | `d528aed`、`9ff563b` | `rust-directory-picker-reference-parity.spec.ts` old/new 各 1/1；`rust-actions-parity-ui.spec.ts` old/new 各 9/9 | old/new 实际打开移动目标选择器，比较触发器、面包屑、子目录、深层路径、空目录图标、目标点击/Escape 关闭及 `defaultPrevented=false` | 局部 PASS |

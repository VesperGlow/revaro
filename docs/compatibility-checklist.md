...
| 移动/复制 loading、失败与成功反馈 | 8、15 | `8bbbd22`、`f953af8`、`83f7738`、`8c12a23`、`1a3776c`（复制失败/命名 parity） | `rust-transfer-dialog-reference-parity.spec.ts` old/new 双上下文 2/2；`rust-mutation-feedback-order-reference-parity.spec.ts` old/new 7/7；`rust-copy-reference-parity.spec.ts` old/new 3/3；`rust-copy-api-reference-parity.spec.ts` 1/1；`cargo xtask check` 通过 | 延迟移动 PATCH 请求期间实际点击 old/new 遮罩，比较弹窗卸载结果；同一 500 响应比较“已移动 0 项，1 项失败：文件：move failed”；700ms children 延迟确认成功 toast 等待目录刷新；按旧版媒体更多菜单入口完成复制、目标目录选择、源文件保留、副本刷新和“已复制 1 项”；复制 409/500 关闭弹窗并保留源文件；真实 API 同名后缀、无效目标和错误分流一致；Rust 初始 busy 限制、失败数量文案和成功反馈时序均已恢复；移动目标排除和完整 loading/错误矩阵仍未完 | 局部 PASS |

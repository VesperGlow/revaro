...
| `[P]` | 预览 | `/preview` content type、图片/音频/视频/文本行为、鉴权、缓存、错误、thumbnail fallback 一致。 | old/new 真实上传同一 TXT、PNG、WAV、WebM 后逐项请求 preview；四类文件的状态、Content-Type、Content-Disposition、Range/Content-Range、If-Range、缓存/安全头和内容结果一致；文本 415、未鉴权 401、图片 thumbnail 成功及文本无 thumbnail 404 也一致。ETag 具体值和 JPEG 压缩字节因独立存储/编码器不同，按 quoted/present、格式和非空结果比较；`rust-preview-reference-parity.spec.ts` old/new 1/1 |
...
| 预览 HTTP 与 thumbnail 回退 | 12、13、15 | `68775ff` | `rust-preview-reference-parity.spec.ts` old/new 1/1 | 真实上传 TXT/PNG/WAV/WebM，逐项对照 preview 类型、inline disposition、Range/If-Range、415/401 错误、图片 thumbnail 与无 thumbnail 分支；独立实例的 ETag 值及 JPEG 编码长度按语义比较 | PASS |

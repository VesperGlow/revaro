...
- `2026-09-15`，old `18180` / new `18184`：实际逐格式上传并解压 TAR、tar.gz/tgz、tar.bz2/tbz2/tbz、tar.xz/txz、tar.zst/tzst、7z，并以有效 ZIP 字节验证 `.rar` 后缀识别；同时记录单文件 `.gz/.bz2/.xz/.zst` 在 reference 中本身失败。每一项均比较任务终态、阶段、进度、错误文案和成功输出文件内容/大小/MIME；Rust 初始 `entry.txt` 为 `text/plain`，旧版为 `text/plain; charset=utf-8`，已恢复 Go MIME 规则。真实 RAR 字节、running/importing 取消及完整冲突/刷新矩阵仍未覆盖；格式矩阵 old/new 1/1，修复提交 `48c34b3`。
...
| `[ ]` | 归档解压 | 支持格式、密码输入任务、冲突/错误、取消、任务中心、完成刷新和安全路径行为一致。 | old/new `rust-archive-real-reference-parity.spec.ts` 现覆盖真实 ZIP 完成/重复冲突、加密 ZIP waiting_input/任务中心/取消、损坏与 `../` 路径错误；新增真实 TAR、tar.gz/tgz、tar.bz2/tbz2/tbz、tar.xz/txz、tar.zst/tzst、7z 格式，以及 `.rar` 后缀识别，均 old/new 1/1 比较任务终态、错误文案、输出内容/大小/MIME。单文件 `.gz/.bz2/.xz/.zst` 的 reference 失败也保持一致；`entry.txt` 的 Go MIME charset 已恢复（`48c34b3`）。旧版真实 `/api/tasks/{id}/input` 的锁内持久化死锁未复制到 Rust；Rust server 集成测试覆盖正常错误密码→重试→正确密码完成。真实 RAR 字节、running/importing 取消、完整冲突/错误/刷新矩阵仍待验；归档项保持未完成 |
...
| 下载/分享/归档 | 12 | `ff43716`（Range）、`74051ee`（ServeContent Range parity）、`85d4a50`（批量 ZIP ID/完整 E2E）、`724aa3d`（归档真实错误/冲突/取消 parity）、`48c34b3`（归档格式/MIME parity）、`d18556d`（UI）、`d257696`（E2E）、`3967289`（公开分享 transport E2E）、`7b278b7`（归档入口双版本） | 单文件、批量 ZIP、分享生命周期、公开分享安全 headers/Range/无效 token、归档入口/任务密码/状态刷新、preview/206/416 已 old/new 实测；ServeContent 的多段 Range、错误边界已恢复；归档真实 ZIP 的重复输出、等待态取消、损坏/穿越错误语义和多格式任务终态/输出/MIME 已恢复；大文件/缓存条件请求/媒体 seek、真实 RAR 字节、running/importing 取消和归档完整状态矩阵仍未完 | download/share/action parity trace；`rust-download-reference-parity.spec.ts` old/new 2/2（含 multipart 大文件/日期条件）；`rust-batch-download-reference-parity.spec.ts` old/new 1/1（真实 ZIP 内容、重复命名、一次性 token、错误分流）；公开分享 old/new 追加断言；`rust-archive-reference-parity.spec.ts` old/new 1/1；`rust-archive-real-reference-parity.spec.ts` ZIP/密码/错误/安全 4/4，格式矩阵 1/1 | 局部 PASS |

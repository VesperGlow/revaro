...
| 下载/分享/归档 | 12 | `ff43716`（Range）、`74051ee`（ServeContent Range parity）、`d18556d`（UI）、`d257696`（E2E）、`3967289`（公开分享 transport E2E）、`7b278b7`（归档入口双版本） | 单文件、ZIP、分享生命周期、公开分享安全 headers/Range/无效 token、归档入口/任务密码/状态刷新、preview/206/416 已 old/new 实测；ServeContent 的多段 Range、错误边界已恢复；大文件/缓存条件请求/媒体 seek 与视觉状态仍未完 | download/share/action parity trace；`rust-download-reference-parity.spec.ts` old/new 2/2（含 multipart 大文件/日期条件）；公开分享 old/new 追加断言；`rust-archive-reference-parity.spec.ts` old/new 1/1；`rust-archive-real-reference-parity.spec.ts` 真实 ZIP old/new 2/2 | 局部 PASS |

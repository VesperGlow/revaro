...
- `[P]` 当前兼容恢复工作树 HEAD：`3d50af0`；上面的 `e9b6202` 保留为恢复开始时的 Rust 基线，后续每个逻辑模块均以独立提交推进。
...
| 状态 | 条目 | 旧版规范与验收点 | 当前 Rust 初检 |
|---|---|---|---|
| `[ ]` | 文件卡/行 | 文件名、大小、类型、更新时间、目录/媒体/文档标识、thumbnail/cover、fallback 和截断规则一致；方块与列表都验证。回收站目录按 Enter 仍阻止默认事件但不打开；目录名带 `.epub` 仍按目录渲染。 | Rust 有 FileTile/rows 基础；回收站目录 Enter 与目录 `.epub` 的文件类型边界已恢复，视觉和全类型矩阵待对照 |
...
| 文件项回收站键盘语义与类型边界 | 6、15 | `31a8ce7`、`3d50af0` | `rust-file-interaction-parity.spec.ts` old/new 定向各 2/2；`cargo test -p revaro-web` 51/51；WASM check | 实际验证回收站目录 Enter 的无条件 `.prevent`，以及目录名带 `.epub` 时不请求 thumbnail、保持文件夹 fallback/icon | 局部 PASS |

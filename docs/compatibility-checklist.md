...
| `[ ]` | 音频播放器 | `/audio` 元数据、封面 fallback、章节、上一/下一章、时间跳转、进度、播放/暂停、loading/error/retry、一首/多首行为一致。 | old/new media parity 已覆盖章节标识、controls、桌面/移动宽度和起点/末尾 seek 钳制；seek/关闭进度保存时序及关闭时 `keepalive` 最终保存另由 `rust-media-parity-ui.spec.ts` old/new 1/1 验证；播放状态、错误重试和完整持久化矩阵仍待验 |

...
<template>
  <details ref="center" class="task-center">
    ...
    <section class="task-panel">
      <header><div><strong>任务中心</strong><small>{{ active.length?`${active.length} 项进行中 · ${progress}%`:'任务通知' }}</small></div><button @click.prevent.stop="openDownload"><Plus/>新建下载</button></header>
      ...
      <div v-else class="task-list">
        ...
        <section v-if="completed.length" class="task-group completed-group"><h3>最近完成 <span>{{ completed.length }}</span><button @click="clearFinished">清除</button></h3><article v-for="task in visibleCompleted" :key="task.id"><span class="kind">{{ labels[task.type]||task.type }}</span><div><strong :title="task.name">{{ task.name||labels[task.type]||task.id }}</strong><small>{{ status(task) }}</small><i><b class="completed" style="width:100%"></b></i></div><em>100%</em></article><button v-if="completed.length>4" class="expand" @click="showAllCompleted=!showAllCompleted">{{ showAllCompleted?'收起':`展开其余 ${completed.length-4} 项` }}</button></section>

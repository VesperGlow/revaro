...
const download=ref<InstanceType<typeof DownloadCenter>|null>(null)
...
function openTask(task:BackgroundTask){if(task.type==='bt'&&['metadata','waiting','paused'].includes(task.phase))download.value?.openById(task.source_id||task.id);else if(task.status==='waiting_input'&&task.type==='archive_extract'){passwordTask.value=task;password.value='';error.value=''}}
...
function retryTask(task:BackgroundTask){if(task.type==='bt'&&task.source_id)download.value?.retryById(task.source_id);else emit('retry',task)}
</script>
...
<template>
  <details class="task-center">
    ...
    <section class="task-panel">
      <header><div><strong>任务中心</strong><small>{{ active.length?`${active.length} 项运行中 · ${progress}%`:'最近任务' }}</small></div><span><button v-if="terminal.length" @click.prevent="clearFinished">清除完成</button><button @click.prevent="download?.openCreate()"><Plus/>新建下载</button></span></header>

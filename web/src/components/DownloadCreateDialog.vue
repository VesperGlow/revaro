...
function reset(){mode.value='magnet';magnet.value='';directURL.value='';torrentFile.value=null;detail.value=null;selected.value=new Set();error.value='';busy.value=false;destinationId.value=props.parentId}
...
function close(){if(busy.value)return;open.value=false;window.clearTimeout(metadataTimer)}
function onEscape(event:KeyboardEvent){if(event.key==='Escape'&&open.value)close()}
    ...
    <div v-if="open" class="download-backdrop" @pointerdown.self="close">
      <section class="download-dialog" role="dialog" aria-modal="true" aria-label="新建下载" @pointerdown.stop>
        ...
        <template v-if="!detail">
          ...
          <div class="destination-field"><span>保存到</span><DirectoryPicker :model-value="destinationId" :disabled="busy" @change="id=>destinationId=id"/><small>任务创建后将在统一任务中心显示</small></div>

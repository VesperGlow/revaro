...
const props=defineProps<{
  ...
  trashMode?:boolean
  /** list 用于列表视图：图标/封面在左，信息在右。 */
  layout?:'tile'|'list'
  /** cover 让封面按原始比例完整显示（书架使用）。 */
  cover?:boolean
  subtitle?:string
}>()
...
function startHold(item:DriveFile,event:PointerEvent){
  if(!props.selectable||(event.pointerType!=='touch'&&event.pointerType!=='pen'))return
  cancelHold()
...
function finishHold(){cancelHold();if(held)heldResetTimer=window.setTimeout(()=>held=false,500)}
function activate(item:DriveFile){
  if(held){held=false;return}
  if(props.selectable&&isTouch()&&props.selectionMode){emit('select',item);return}
  openItem(item)
}
...
<template>
  <article v-if="layout==='list'" class="file-row" :class="{mutedrow:item.status!=='ready',selected,selectable}" role="button" tabindex="0" :aria-label="`${item.name}，${selected?'已选择':'未选择'}`" @click="activate(item)" @contextmenu.prevent @keydown.enter.prevent="openItem(item)" @keydown.space.prevent="selectable&&$emit('select',item)">
    <button v-if="selectable" class="row-select" :class="{active:selected}" :title="selected?'取消选择':'选择项目'" :aria-label="selected?'取消选择':'选择项目'" :aria-pressed="selected" @click.stop="$emit('select',item)" @keydown.stop><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 4 4L19 6"/></svg></button>
    <div class="row-preview" :class="{'cannot-open':!canOpen(item)}">
    ...
    <div class="row-info">
      ...
      <small v-else-if="trashMode">{{ item.kind==='directory'?'文件夹':formatSize(item.size) }} · 删除于 {{ formatDate(item.deleted_at||item.updated_at) }}</small>
      <small v-else>{{ item.kind==='directory'?'文件夹':formatSize(item.size) }}</small>
    </div>
    <div v-if="$slots.trailing" class="row-trailing"><slot name="trailing" /></div>
  </article>
  <article v-else class="file-card" :class="{mutedrow:item.status!=='ready',selected,'preview-tile':hasPreview(item),'fallback-tile':!hasPreview(item),'folder-tile':item.kind==='directory','document-tile':isEditable(item),'book-tile':isEpub(item),'audio-tile':isAudio(item),'cover-tile':cover}" role="button" tabindex="0" :aria-label="`${item.name}，${selected?'已选择':'未选择'}`" @click="activate(item)" @pointerdown="startHold(item,$event)" @pointerup="finishHold" @pointercancel="finishHold" @pointermove="moveHold" @contextmenu.prevent @keydown.enter.prevent="openItem(item)" @keydown.space.prevent="selectable&&$emit('select',item)">
    <button v-if="selectable" class="card-select" :class="{active:selected}" :title="selected?'取消选择':'选择项目'" :aria-label="selected?'取消选择':'选择项目'" :aria-pressed="selected" @click.stop="$emit('select',item)" @keydown.stop>
    ...
    <div class="card-info"><strong :title="item.name">{{ item.name }}</strong><small v-if="subtitle">{{ subtitle }}</small><small v-else-if="trashMode">{{ item.kind==='directory'?'文件夹':formatSize(item.size) }} · 删除于 {{ formatDate(item.deleted_at||item.updated_at) }}</small><small v-else>{{ item.kind==='directory'?'文件夹':formatSize(item.size) }}</small></div>
    <slot />
  </article>
</template>

...
import { Cloud, Database, HardDrive } from '@lucide/vue'
...
<template>
  <details ref="panel" class="system-status" :class="status?.status||'pending'">
    ...
    <section class="status-panel">
      <header>
        <div><strong>系统状态</strong><small>存储使用量与核心服务概览</small></div>
      ...
      <div v-else class="status-grid">
        ...
        <ServiceCard title="网盘存储使用量" :detail="`${status.storage.file_count} 个文件 · 回收站 ${formatSize(status.storage.trash_bytes)}`" :badge="formatSize(status.storage.bytes)" :tone="tone(status.storage.status)"><template #icon><Cloud /></template></ServiceCard>

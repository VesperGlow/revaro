...
const statusText=(job:ArchiveJob)=>job.status==='queued'?'等待解压':job.status==='downloading'?'正在读取压缩包':job.status==='checking'?'正在检查内容':job.status==='extracting'?'正在解压':job.status==='importing'?'正在保存到网盘':job.status==='done'?'已完成':job.error||'解压失败'
    ...
    <section class="archive-popover">
      ...
      <div class="archive-list">
        <article v-for="job in jobs" :key="job.id">
          <span class="task-icon">▦</span><div><strong :title="job.name">{{ job.name }}</strong><small>{{ job.message||statusText(job) }} · {{ statusText(job) }}</small><i><b :class="job.status" :style="{width:`${job.progress}%`}"></b></i></div><em>{{ job.progress }}%</em>
        </article>
      </div>
    </section>
  </details>

...
<template>
  <div ref="shell" class="video-player-shell" :class="{'cursor-hidden':cursorHidden}" tabindex="0" @pointermove="$event.pointerType==='mouse'&&showControls()" @pointerdown="pointerType=$event.pointerType" @keydown.tab.capture="showControls()" @keydown="onKey">
    ...
    <VideoStatusOverlay :item-name="item.name" :controls-visible="controlsVisible" :playing="playing" :starting="starting" :error="error" :timeline-position="timelinePosition" :buffering="buffering" :format-time="formatTime" @close="emit('close')" @toggle-playback="togglePlayback" @retry="retryPlayback" />

...
test('创建目录、真实上传、移动、删除、回收站和恢复',async({page})=>{
  ...
  const directoryName=`e2e-target-${suffix}`
  const fileName=`hello-e2e-${suffix}.txt`
  await login(page)
  await page.getByRole('button',{name:'新建文件夹'}).click()
  await page.getByRole('dialog').getByPlaceholder('文件夹名称').fill(directoryName)
  await page.getByRole('dialog').getByRole('button',{name:'创建'}).click()
  await expect(page.locator('.file-card').filter({hasText:directoryName})).toBeVisible()

  await page.locator('input[type=file]').first().setInputFiles({name:fileName,mimeType:'text/plain',buffer:Buffer.from('real revaro upload\n')})
  const uploaded=page.locator('.file-card').filter({hasText:fileName})
  await expect(uploaded).toBeVisible({timeout:20_000})

  await selectCard(page,fileName)
  await page.getByRole('toolbar',{name:'所选项目操作'}).getByRole('button',{name:'移动'}).click()
  await page.locator('.directory-trigger').click()
  await page.getByRole('region',{name:'选择目标目录'}).getByRole('button',{name:directoryName}).click()
  await page.locator('.directory-trigger').click()
  await page.locator('.move-copy-dialog').getByRole('button',{name:'移动'}).click()
  await expect(uploaded).toBeHidden()

  await page.locator('.file-card').filter({hasText:directoryName}).click()
  await selectCard(page,fileName)
  ...
  await expect(page.locator('.file-card').filter({hasText:fileName})).toBeHidden()
...
test('多选文件通过一次 ZIP 下载且 CSP 保持禁止 frame',async({page})=>{
  const suffix=crypto.randomUUID()
  const firstName=`batch-one-${suffix}.txt`,secondName=`batch-two-${suffix}.txt`
  ...
  await page.locator('input[type=file]').first().setInputFiles([
    {name:firstName,mimeType:'text/plain',buffer:Buffer.from('batch one\n')},
    {name:secondName,mimeType:'text/plain',buffer:Buffer.from('batch two\n')},
  ])
  await expect(page.locator('.file-card').filter({hasText:firstName})).toBeVisible({timeout:20_000})
  await expect(page.locator('.file-card').filter({hasText:secondName})).toBeVisible({timeout:20_000})
  await selectCard(page,firstName)
  await selectCard(page,secondName)
